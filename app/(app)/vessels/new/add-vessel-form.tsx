...
import { SanctionsListInput } from "@/components/app/sanctions-list-input";
...
export function AddVesselForm({
  ...
  // user picks a flag. The user can still override it (e.g. type "LBR"
        ...
        <div className="flex flex-col gap-4">
                ...
                <FormField label="Flag" required error={fieldErrors.flagCountryId?.[0]}>
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      // moment a flag is picked. Empty selection clears
          ...
          <Card>
            ...
            <div className="p-6">
              {/* `SanctionsListInput` posts a JSON-encoded hidden input
                  named "sanctions"; createVesselAction parses it and the
                  Zod schema validates each entry. */}
              <SanctionsListInput />
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="grid gap-4 p-6">
              ...
              {/* Row 3: On Sale flag + Listed-at Date. The action reads
                  `isOnSale` via the checkbox helper (`on` → true, absent
                  → false) and `onSaleAt` as a date string. The "ON SALE"
                  tag on the fleet's vessel table reads this same flag. */}
              <div className="grid gap-4 md:grid-cols-4">
  ...
  return (
    ...
    >
      ...
      {/* Vessel name — matches prototype's `.vessel-preview-name` →
          ...
          Uses `text-foreground` so the name matches the color of the
          section group titles (e.g. "Vessel Identification") on the
          left rail rather than the primary accent. */}
