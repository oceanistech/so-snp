...
export function CreateFleetForm({
  ...
  const filteredVessels = React.useMemo(() => {
    ...
    if (!q) return attachableVessels;
    return attachableVessels.filter(
      (v) =>
  ...
  }, [attachableVessels, vesselFilter]);
          ...
          <div className="grid gap-4 p-4 pt-0 md:grid-cols-3">
            <FormField label="Owner">
              <input
                ...
                value={ownerLabel}
                readOnly
                className="h-9 w-full rounded-md border border-input bg-muted/40 px-3 text-[13px] text-muted-foreground"
              />
            </FormField>
